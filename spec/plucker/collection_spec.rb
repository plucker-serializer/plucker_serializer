...
describe Plucker::Collection do
  ...
  context 'instantiation types' do
    it 'from database' do
      ...
      expect(Plucker::Collection.new(Foo.all,
                                     serializer: FooSerializer).to_h).to eq([{ name: foo.name, address: foo.address },
                                                                             { name: foo2.name,
  ...
  context 'custom options' do
    it 'custom serializer' do
      ...
      expect(Plucker::Collection.new(Foo.all,
                                     serializer: FooCustomSerializer).to_h).to eq([{ name: foo.name },
                                                                                   { name: foo2.name }])
    end
  end
  ...
  context 'plucking' do
    it 'base' do
      ...
      expect(FooPluckedSerializer.pluckable?).to eq(true)
      expect(Plucker::Collection.new(Foo.all,
                                     serializer: FooPluckedSerializer).to_h).to eq([{ name: foo.name },
                                                                                    { name: foo2.name }])
    end
  end
end
